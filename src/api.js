...
module.exports = {
      ...
      if (
        ...
        self
          ...
          .then(function (result) {
            ...
            if (
              ...
              self.currentState.dynamicVariables.organization_name =
                result.data.attributes.name;
            } else {
        ...
        self
          ...
          .then(function (result) {
            ...
            if (
              self.config.polling &&
              self.config.servicetypeid_polling &&
              self.config.servicetypeid_polling !== ""
            ) {
              self.log(
                "debug",
                "Getting Plan Id from Service Type for Polling...",
              );
              self
                ...
                .catch(function (message) {
                  self.log(
                    "error",
                    "Error getting Plan Id from Service Type for Polling: " +
                      message,
                  );
        ...
        self.log(
          "error",
          "Error Initializing: Application ID and Secret Key are required.",
        );
        self.updateStatus(
          InstanceStatus.BadConfig,
          "Application ID and Secret Key are required.",
        );
      }
    } catch (error) {
  ...
  processServicesData: function (result) {
    ...
    try {
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error processing Services data: " + error,
      );
    }
  },
  ...
  processPlansData: function (result) {
    ...
    try {
      ...
      for (let j = 0; j < result.length; j++) {
        ...
        planListObj.serviceTypeId =
          result[j].relationships.service_type.data.id;
        let serviceObj = services.find(
          (s) => s.id === planListObj.serviceTypeId,
        );
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error processing Plans data: " + error,
      );
    }
  },
  ...
  processPlanIncludedData: function (result) {
    ...
    try {
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error processing Plan Included data: " + error,
      );
    }
  },
            ...
            if (result.data.links.controller === null) {
              ...
              self
                ...
                .catch(function (message) {
                  ...
                  self.updateStatus(
                    InstanceStatus.ConnectionFailure,
                    "Error toggling control: " + message,
                  );
                });
            } else {
              //someone is in control, so let's check to see who it is
              if (
                result.data.links.controller ===
                self.currentState.internal.currentController
              ) {
                  ...
                  .then(function (result) {
                    ...
                    self
                      ...
                      .catch(function (message) {
                        self.log(
                          "error",
                          "Error togggling control: " + message,
                        );
                        self.updateStatus(
                          InstanceStatus.ConnectionFailure,
                          "Error toggling control: " + message,
  ...
  controlLive: function (serviceTypeId, planId, direction) {
    ...
    try {
      ...
      switch (direction) {
        ...
        case "previous":
          url =
            baseUrl + "/go_to_previous_item?include=items,current_item_time";
          break;
        case "reset":
  ...
  getCurrentLive: function () {
    ...
    try {
      ...
      if (serviceTypeId && planId) {
        ...
        self
          ...
          .catch(function (message) {
            ...
            self.updateStatus(
              InstanceStatus.ConnectionFailure,
              "Error Getting LIVE data: " + message,
            );
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error getting current live data: " + error,
      );
    }
  },
    ...
    try {
      if (result.errors) {
        ...
        if (result.data && result.data.attributes) {
          self.currentState.dynamicVariables["plan_title"] =
            result.data.attributes.title;
          self.currentState.dynamicVariables["plan_series"] =
            result.data.attributes.series_title;
        }

        let items = result.included;

        let currentItemTimeId =
          result.data.relationships.current_item_time.data &&
          result.data.relationships.current_item_time.data.id;
        let currentItemTime = result.included.find(
          (res) => res.type === "ItemTime" && res.id === currentItemTimeId,
        );
        let currentItemId =
          currentItemTime &&
        ...
        if (currentItemId) {
          ...
          if (item.type === "ItemTime") {
            ...
            let dtTimeStarted = new Date(
              currentItemTime.attributes.live_start_at,
            );
            let timeStarted_formatted = dtTimeStarted.toLocaleTimeString();
            let dtTimeShouldFinish = new Date(
              dtTimeStarted.getTime() + item.attributes.length * 1000,
            );
            ...
            if (item.id !== self.lastPlanItemId) {
              ...
              self.lastPlanItemId = item.id;
              self.startPlanItemTimeRemainingInterval(
                dtTimeStarted,
                dtTimeShouldFinish,
              );
            }
            ...
            let timeLength_formatted =
              Math.floor(timeLength / 60) +
              ":" +
              ("0" + Math.floor(timeLength % 60)).slice(-2);

            //get the length of only the ones without type of header
            let itemsWithoutHeader = items.filter(
              (item) =>
                item.attributes.item_type !== "header" && item.type === "Item",
            );

            if (itemsWithoutHeader) {
              let indexWithoutHeader = itemsWithoutHeader.findIndex(
                (i) => i.id === currentItemId,
              );
              self.currentState.dynamicVariables["plan_index"] =
                indexWithoutHeader + 1;
              self.currentState.dynamicVariables["plan_length"] =
                itemsWithoutHeader.length;
            } else {
              self.currentState.dynamicVariables["plan_index"] = index + 1;
              self.currentState.dynamicVariables["plan_length"] = items.length;
            }

            self.currentState.dynamicVariables["plan_currentitem"] =
              item.attributes.title;
            self.currentState.dynamicVariables["plan_currentitem_time_length"] =
              timeLength_formatted;
            self.currentState.dynamicVariables[
              "plan_currentitem_time_started"
            ] = timeStarted_formatted;
            self.currentState.dynamicVariables[
              "plan_currentitem_time_shouldfinish"
            ...
            self.getItemNotes(item.id).then((notes) => {
              ...
              self.planItemNoteCategories.forEach((category) => {
                let variableId =
                  "plan_currentitem_notes_" + self.sanitize(category.name);
              ...
              notes.forEach((note) => {
                ...
                let variableId =
                  "plan_currentitem_notes_" + self.sanitize(categoryName);
            ...
            if (index < items.length) {
              for (let i = index + 1; i < items.length; i++) {
                ...
                if (
                  items[i].attributes.item_type == "item" ||
                  items[i].attributes.item_type == "song"
                ) {
                  //if the item type is a song, grab the key name and other stuff
                  if (item.attributes.item_type == "song") {
                    self.currentState.dynamicVariables["plan_nextitem_key"] =
                  ...
                  if (items[i].attributes.description) {
                    self.currentState.dynamicVariables[
                      "plan_nextitem_description"
                    ] = items[i].attributes.description;
                  } else {
                    self.currentState.dynamicVariables[
                      "plan_nextitem_description"
                    ] = "";
                  ...
                  self.getItemNotes(items[i].id).then((notes) => {
                    ...
                    self.planItemNoteCategories.forEach((category) => {
                      let variableId =
                        "plan_nextitem_notes_" + self.sanitize(category.name);
                      if (!variableObj[variableId]) {
                        variableObj[variableId] = "";
                      }
                    });
                    ...
                    notes.forEach((note) => {
                      let categoryName = note.attributes.category_name;
                      let variableId =
                        "plan_nextitem_notes_" + self.sanitize(categoryName);
                      let variableName =
                        "Plan Next Item Notes - " + categoryName;
          ...
          if (preServiceItems) {
            preServiceItemsLength = preServiceItems.reduce(
              (a, b) => a + b.attributes.length,
              0,
            );
          }
          ...
          self.currentState.dynamicVariables["plan_index"] = "";
          self.currentState.dynamicVariables["plan_currentitem_time_length"] =
            ""; //show the service time start here
          ...
          self.currentState.dynamicVariables[
            "plan_currentitem_time_remaining_seconds"
          ] = "";
          self.currentState.dynamicVariables[
            "plan_currentitem_time_remaining"
          ] = ""; //show the countdown to service start here (it's not a countdown to the service start time but a countdown to the service start time minus whatever pre service item lengths exist)
          self.currentState.dynamicVariables["plan_currentitem"] =
            "START OF SERVICE";
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error processing LIVE data: " + error,
      );
    }
  },
  ...
  getItemNoteCategories: function () {
    ...
    if (!self.lastServiceTypeId) {
      return Promise.reject(
        "No Service Type Id set. Please select a service type first.",
      );
    }
  ...
  startPlanItemTimeRemainingInterval: function (
    dtTimeStarted,
    dtTimeShouldFinish,
  ) {
    let self = this;
    ...
    self.ITEM_TIME_REMAINING_INTERVAL = setInterval(function () {
      try {
        ...
        let timeRemaining = Math.floor(
          (dtTimeShouldFinish.getTime() - dtNow.getTime()) / 1000,
        );
        let timeRemaining_formatted =
          Math.floor(Math.abs(timeRemaining) / 60) +
          ":" +
          ("0" + Math.floor(Math.abs(timeRemaining) % 60)).slice(-2);
        if (timeRemaining < 0) {
        ...
        self.currentState.dynamicVariables["plan_currentitem_time_remaining"] =
          timeRemaining_formatted;
        self.checkVariables();
        self.checkFeedbacks();
      } catch (error) {
  ...
  getPlanIdOfServiceType: function (serviceTypeId) {
    ...
    try {
      ...
      return new Promise(function (resolve, reject) {
        self
          ...
          .catch(function (message) {
            self.log(
              "error",
              "Error getting Plan Id from Service Type: " + message,
            );
            self.updateStatus(InstanceStatus.ConnectionFailure, message);
          });
      });
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error getting next Plan Id by Service Type Id: " + error,
      );
    }
  },
  ...
  getServiceIdFromPlanId: function (planId) {
    ...
    try {
      let planObj = self.currentState.internal.plans_list.find(
        (p) => p.id === planId,
      );
      if (planObj) {
      ...
      self.updateStatus(
        InstanceStatus.ConnectionFailure,
        "Error getting Service Type Id from Plan Id: " + error,
      );
    } finally {
  ...
  processTeamPositions: function (result) {
    ...
    try {
      ...
      data.forEach((person) => {
        ...
        const team = included.find(
          (res) => res.type === "Team" && res.id === teamId,
        );
      ...
      newScheduledPeople.sort((a, b) => {
        // Define status priority: C=0, U=1, D=2
        const statusOrder = { C: 0, U: 1, D: 2 };
      ...
      newScheduledPeople.forEach((person) => {
        const teamNameId = person.teamName
          .replace(/[^a-zA-Z0-9]/g, "")
          .toLowerCase();
        const positionNameId = person.positionName
          .replace(/[^a-zA-Z0-9]/g, "")
          .toLowerCase();
        const compositeKey = `${teamNameId}-${positionNameId}`;
        choicesCounters[compositeKey] =
          (choicesCounters[compositeKey] || 0) + 1;
        const id = `${compositeKey}-${choicesCounters[compositeKey]}`;
        const label =
          `${person.teamName} - ${person.positionName}` +
          (choicesCounters[compositeKey] > 1
            ? ` (${choicesCounters[compositeKey]})`
            : "");
      ...
      const choicesChanged =
        JSON.stringify(newChoicesPositions) !==
        JSON.stringify(self.CHOICES_POSITIONS || []);
  ...
  async getPersonPhoto(personPhotoUrl) {
    ...
    try {
      ...
      if (!response.ok) {
        self.log(
          "error",
          `Error getting person photo: ${response.status} ${response.statusText}`,
        );
        return null;
      }
      ...
      const fileType = await import("file-type").then((module) =>
        module.fileTypeFromBuffer(buffer),
      );
